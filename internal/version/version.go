...
func GetVersion(s string) *go_version.Version {
	v := strings.Split(s, ".darwin")
	vv := go_version.Must(go_version.NewVersion(strings.TrimPrefix(v[0], "go")))
	return vv
}

// Exists checks if given archive name exists in the specified download directory.
func Exists(archiveName, downloadDir string) (bool, error) {
